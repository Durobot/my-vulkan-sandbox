// Run-time dynamic linking with Vulkan loader dynamic library (vulkan-1.dll on Windows, libvulkan.so on Linux) example.
...
// This method is refered to as "Indirectly Linking to the Loader" in the excerpt from LoaderApplicationInterface.md below.
// 
// Run-time dynamic linking essentially means calling OS functions to load the Vulkan loader library, getting the address of
// Vulkan loader's vkGetInstanceProcAddr() function by using OS functions, then calling it to get addresses of other Vulkan functions.
// Contrast and compare to load-time dynamic linking, which creates a "baked in" connection between the executable and
// the library, which means that the executable would not even run if the OS can't find the library.
// With load-time dynamic linking, however, Vulkan functions are available without the need to use other means to get their addresses.
//
// From
...
//
// 3. Indirectly Linking to the Loader (the method used by this example program)
...
// To get the best possible performance in a Vulkan application, the application should set up its own dispatch table (a table of pointers)
// for every Vulkan API entry-point. For every instance-level Vulkan command in the dispatch table, the function pointer should be queried
// and filled in by using the results of vkGetInstanceProcAddr. Additionally, for every device-level Vulkan command, the function pointer
// should be queried and filled in using the resulsts of vkGetDeviceProcAddr.
//
// Why do this?
// The answer comes in how the call chain of instance functions are implemented versus the call chain of a device functions. Remember,
// a Vulkan instance is a high-level construct used to provide Vulkan system-level information. Because of this, instance functions need
// to be broadcast to every available driver on the system. The following diagram shows an approximate view of an instance call chain
// with three enabled layers:
// +-------------+   +---------------+   +---------+   +---------+   +---------+   +---------------+                   +--------+
// | Vulkan      |-->| Vulkan        |-->| Vulkan  |-->| Vulkan  |-->| Vulkan  |-->| Vulkan        |------------------>| Vulkan |
// | Application |   | loader        |   | Layer A |   | Layer B |   | Layer C |   | loader        |    +--------+     | Driver |
// +-------------+   | trampoline fn |   +---------+   +---------+   +---------+   | terminator fn |--->| Vulkan | ... +--------+
//                   +---------------+                                             +---------------+    | Driver |
//                                                                                                      +--------+
// This is also how a Vulkan device function call chain looks if queried using vkGetInstanceProcAddr. On the other hand, a device
// function doesn't need to worry about the broadcast because it knows specifically which associated driver and which associated
// physical device the call should terminate at. Because of this, the loader doesn't need to get involved between any enabled
// layers and the driver. Thus, using a loader-exported Vulkan device function, the call chain in the same scenario as above would
// look like:
// +-------------+   +---------------+   +---------+   +---------+   +---------+   +--------+
// | Vulkan      |-->| Vulkan        |-->| Vulkan  |-->| Vulkan  |-->| Vulkan  |-->| Vulkan |
// | Application |   | loader        |   | Layer A |   | Layer B |   | Layer C |   | Driver |
// +-------------+   | trampoline fn |   +---------+   +---------+   +---------+   +--------+
//                   +---------------+
// An even better solution would be for an application to perform a vkGetDeviceProcAddr call on all device functions.
// This further optimizes the call chain by removing the loader all-together under most scenarios:
// +-------------+   +---------+   +---------+   +---------+   +--------+
// | Vulkan      |-->| Vulkan  |-->| Vulkan  |-->| Vulkan  |-->| Vulkan |
// | Application |   | Layer A |   | Layer B |   | Layer C |   | Driver |
// +-------------+   +---------+   +---------+   +---------+   +--------+
// Also, notice if no layers are enabled, the application function pointers point _directly to the driver_.
// With many function calls, the lack of indirection in each adds up to non-trivial performance savings.
...
// These macros are used to get Vulkan function addresses and check
// that we've got them. This is not how I would do it in any production-ready
// piece of software, as declaring variables in macros is not the best idea, IMO.
#define GET_FN(fn) \
PFN_##fn fn = (PFN_##fn)LoadProcAddress(vk_lib, #fn); \
if(fn == NULL) \
{ \
    printf("Could not get address of function %s\n", #fn); \
    return -2; \
}

#define GET_GLOBAL_LEVEL_FN(fn) \
PFN_##fn fn = (PFN_##fn)vkGetInstanceProcAddr(NULL, #fn); \
if(fn == NULL) \
{ \
    printf("Could not get address of global level function %s\n", #fn); \
    return -3; \
}

#define GET_INSTANCE_LEVEL_FN(fn) \
PFN_##fn fn = (PFN_##fn)vkGetInstanceProcAddr(vk_instance, #fn); \
if(fn == NULL) \
{ \
    printf("Could not get address of instance level function %s\n", #fn); \
    return -4; \
}

int main()
{
    // Vulkan Runtime library (Vulkan Loader)
#if defined(VK_USE_PLATFORM_WIN32_KHR)
    HMODULE vk_lib = LoadLibrary("vulkan-1.dll");
    #define LoadProcAddress GetProcAddress
#elif defined(VK_USE_PLATFORM_XCB_KHR) || defined(VK_USE_PLATFORM_XLIB_KHR)
    void* vk_lib = dlopen("libvulkan.so", RTLD_LAZY);
    #define LoadProcAddress dlsym
#endif
    if(vk_lib == NULL)
    {
        printf("Could not load Vulkan dynamic library\n");
        return -1;
    }
    printf("Vulkan dynamic library loaded\n");

    // Get address of the only function Vulkan library _must_ export, vkGetInstanceProcAddr
    GET_FN(vkGetInstanceProcAddr)
    // Using vkGetInstanceProcAddr, get addresses of the three functions we need to create Vulkan instance.
    // Since we don't have a Vulkan instance yet, we pass NULL as a Vulkan instance to vkGetInstanceProcAddr
    // (see definition of GET_GLOBAL_LEVEL_FN above - this is why we call these functions "global level functions").
    GET_GLOBAL_LEVEL_FN(vkCreateInstance)
    GET_GLOBAL_LEVEL_FN(vkEnumerateInstanceExtensionProperties)
    GET_GLOBAL_LEVEL_FN(vkEnumerateInstanceLayerProperties)
    ...
    // Now that we have created the Vulkan instance, we can get addresses of
    // all the functions we need, by providing the pointer to the instance to vkGetInstanceProcAddr
    GET_INSTANCE_LEVEL_FN(vkDestroyInstance)
    GET_INSTANCE_LEVEL_FN(vkEnumeratePhysicalDevices)
    GET_INSTANCE_LEVEL_FN(vkGetPhysicalDeviceProperties)
    GET_INSTANCE_LEVEL_FN(vkGetPhysicalDeviceFeatures)
    GET_INSTANCE_LEVEL_FN(vkGetPhysicalDeviceQueueFamilyProperties)
    GET_INSTANCE_LEVEL_FN(vkCreateDevice)
    GET_INSTANCE_LEVEL_FN(vkGetDeviceProcAddr)
    GET_INSTANCE_LEVEL_FN(vkEnumerateDeviceExtensionProperties)
    ...
    dlclose(vk_lib);
#endif
	printf("Vulkan dynamic library unloaded\n");
    return 0;
}
